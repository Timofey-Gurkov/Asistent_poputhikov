...
# Список локаций для экрана карты (отображаются в выпадающем меню).
MAP_LOCATIONS = ["ГУК УрФУ", "НВК"]

# Маппинг: название локации → имя PNG-файла (только латиница!).
# Меняйте этот словарь, чтобы добавить новые локации.
# Файлы должны лежать в папке assets/maps/ с такими именами.
MAP_FILES = {
...
def main(page: ft.Page):
    ...
    def map_src(location_name: str) -> str:
        ...
        В Flet веб-билде файлы из папки assets/ доступны по URL
        без префикса 'assets/', то есть assets/maps/guk_urfu.png
        отдаётся по адресу maps/guk_urfu.png."""
    ...
    def show_map_screen():
        ...
        def update_map(e):
            ...
            if selected:
                map_image.src = map_src(selected)
                map_image.update()
